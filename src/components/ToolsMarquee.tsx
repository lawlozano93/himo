...
// Mobile component with tap-to-show tooltip
    ...
    return (
        <div className="grid grid-cols-4 gap-4 py-4">
            {tools.map((tool, index) => {
                const isTapped = tappedTool === index;

                return (
                    <div
                        key={tool.name}
                        className={`relative flex flex-col items-center gap-2 transition-opacity duration-500 ${mounted ? "opacity-100" : "opacity-0"}`}
                        onClick={() => setTappedTool(isTapped ? null : index)}
                    >
                        <div className={`w-14 h-14 rounded-full flex items-center justify-center transition-all duration-200 ${isTapped ? "bg-[#ecf95a] scale-105" : "bg-[#f4f4f4]"}`}>
